...
** Hailing Frequencies or comms
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun comms ()
       ...
       (window-number-select 4)
       (find-file (concat org-directory "/work/world-domination.org"))
     ...
     (global-set-key [C-M-s-f10] 'all-channels-open)
     (global-set-key [C-f10] 'comms)
   #+END_SRC
...
**** sw1nn-cider-perspective or Engineering
     ...
     #+BEGIN_SRC emacs-lisp
       ...
       ;; formerly sw1nn-cider-perspective
       (defun engineering ()
         (interactive)
         (delete-other-windows)
         (split-window-below)
       ...
       (global-set-key [C-f12] 'engineering)
     #+END_SRC
...
*** deft
    ...
    And we want C-f5 to be the key to get us to the top level search
    list. The f5s are where we put our text mode things.

    #+BEGIN_SRC emacs-lisp
      (global-set-key [C-f5] 'deft)
    #+END_SRC
       ...
       #+BEGIN_SRC emacs-lisp
         (setq org-agenda-custom-commands
               '(("D" "Doing Now" todo "DOING|WAITING|BLOCKED|CCC"
                  ((org-agenda-overriding-columns-format "%60ITEM(Task) %8Owner %8Effort(Estimated Effort){:} %CLOCKSUM")x
                   (org-agenda-view-columns-initially t)))
                 ("F" "Doing Future" tags "Owner=\"Bruce\"")
                 ("C" "Cake Countdown" agenda "" ((org-agenda-ndays 7)
...
****** Ops
       ...
       To go with comms and engineering, obviously we need
       ops. Otherwise how are we supposed to know what is going on?

       #+BEGIN_SRC emacs-lisp
         (global-set-key [C-f11] 'org-agenda)
       #+END_SRC
...
***** Capture Hot Key
      ...
      Let's capture things with a quick f5.
