...
**** showcss-mode
     ...
     I like the look of showcss-mode (soon to be renamed to show-css
     according to [[http://twitter.com/sanityinc][Steve Purcell]] according to [[https://twitter.com/sanityinc/status/395948249678622720][this tweet]]).

     Because there is a mismatch between the MELPA package name and
     the mode, we need to do a bit more work to install and load it.

     We also need to install dom.el directly as it showcss-mode
     doesn't list it as a dependency.
     
     #+BEGIN_SRC emacs-lisp
       (maybe-install-and-require 'dom)
       (when (not (package-installed-p 'showcss-mode))
         (package-install 'showcss-mode))
       (require 'show-css)
     #+END_SRC
