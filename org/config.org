...
*** clojure
    ...
    _*ALL OF THIS CLOJURE STUFF IS HORRIBLY BROKEN ATM*_
    ...
    clojure-mode and clojure-test-mode have mostly stayed the same.

    #+BEGIN_SRC emacs-lisp
      ;; pkg-info needs the pre 2.0.0 dash.el which has breaking
      ;; changes. We'll try installing it manually here.
      ;; (maybe-install-and-require 'pkg-info)
    ...
    For now we'll still use nrepl.el and see how things work out with
    cider.el.

    #+BEGIN_SRC emacs-lisp
      ;; (maybe-install-and-require 'cider)
      ;; (add-hook 'cider-mode-hook lisp-hooks)
    #+END_SRC

    We want to have eldoc goodness in the nrepl buffer
