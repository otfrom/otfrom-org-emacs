...
**** Refiling messages
     ...
     #+BEGIN_SRC emacs-lisp
       (setq mu4e-refile-folder
             (lambda (msg)
               (message "Message contact: %s" (mu4e-message-field msg :to))
               (cond
                ;; work email
                ((string-match-p "/mastodonc/" (mu4e-message-field msg :maildir)) "/mastodonc/all")
                
                ;; personal email
                ((string-match-p "/otfrom/" (mu4e-message-field msg :maildir)) "/otfrom/all")
                
                ;; by default things go into /otfrom/all
       ...
       (setq mu4e-trash-folder
             (lambda (msg)
               (message "Message contact: %s" (mu4e-message-field msg :to))
                      (cond
                       ;; work email
                       ((string-match-p "/mastodonc/" (mu4e-message-field msg :maildir)) "/mastodonc/trash")
                       
                       ;; personal email
                       ((string-match-p "/otfrom/" (mu4e-message-field msg :maildir)) "/otfrom/trash")
                       
                       ;; by default things go into /otfrom/trash
                       ;; important to have a catch-all at the end!
                       (t  "/otfrom/trash"))))
